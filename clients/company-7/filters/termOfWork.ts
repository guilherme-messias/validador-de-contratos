export default function termOfWork(
  ...
  if (position.includes("COMERCIAL") || position.includes("VENDAS")) {
    ...
    admissionKit += `Incluir no kit de admissão: ADITIVO COMERCIAL NEXTIOS + POLÍTICA COMERCIAL NEXTIOS - 36760\n`;
  }

  if (position.includes("PRE VENDAS")) {
    contractCodes += `ADITIVO LOCAWEB RV TSP + POLÍTICA TSP - 36759\n`;
    admissionKit += `Incluir no kit de admissão: ADITIVO LOCAWEB RV TSP + POLÍTICA TSP - 36759\n`;
  }
